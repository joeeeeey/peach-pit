...
export default class UploaderArea extends React.Component {
  ...
  uploadImg = () => {
    ...
    axios.post(this.imgUploadUrl, fd).then(res => {
      this.setState({ loading: false });
      ...
      <Spin tip="正在努力上传中，请稍等.." spinning={this.state.loading}>
        <div style={containerStyle}>
          <div
            ...
            onMouseLeave={this.handlerUploaderMouseLeave}
            onMouseOver={this.handlerUploaderHover}>
            <button style={this.UploaderButtonStyle()} className="btn">
              上传图片
            </button>
            <input
