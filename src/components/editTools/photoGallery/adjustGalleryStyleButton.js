...
export default class AdjustGalleryStyleButton extends React.Component {
  constructor(props, context) {
    super(props);
    this.positionStyle = this.props.positionStyle || { zIndex: 30, position: 'absolute', left: '1%', top: 10, "borderRadius": "10%", "background": "#303233" }
    ...
    return (
      <Menu >
        <Menu.Item key={'imgContainerMargin'}>
          <div>
            <h4>间距</h4>
            <Slider max={30} onAfterChange={this.commonHandleChange('imgContainerMargin')} defaultValue={this.props.imgContainerMargin} style={{ minWidth: 256 }} />
          </div>
        </Menu.Item>
        <Menu.Item key={'galleryWidth'}>
          <div>
            <h4>画幅</h4>
            <Slider min={30} onAfterChange={this.commonHandleChange('galleryWidth')} defaultValue={this.props.galleryWidth} style={{ minWidth: 256 }} />
          </div>
        </Menu.Item>
        <Menu.Item key={'intensity'}>
          <div>
            <h4>密集度(单行容纳的相对最大列数)</h4>
            <Slider onAfterChange={this.commonHandleChange('intensity')} min={2} max={4} defaultValue={this.props.intensity} style={{ minWidth: 256 }} />
          </div>
        </Menu.Item>
        <Menu.Item key={'updateGalleryStyle'}>
          <div style={{ textAlign: 'center' }}>
            <Button style={{ marginBottom: 5, }} onClick={this.updateGalleryStyle}> 确认</Button>
          </div>
        </Menu.Item>
      </Menu>
    )
