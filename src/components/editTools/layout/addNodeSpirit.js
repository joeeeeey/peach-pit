...
export default class AddNodeSpirit extends Component {
  ...
  getNodeDataByName = nodeName => {
    ...
    switch (nodeName) {
      case "文本框":
        return JSON.parse(
          '{"native":false,"nodeName":"TextArea","props":{"deltaDeltaValue":[{"insert":"在此输入标题","attributes":{"font":"serif"}},{"insert":"\\n","attributes":{"align":"center","header":1}}],"readOnly":false}}'
        );
      case "图片":
        return JSON.parse(
          '{"native": false, "nodeName": "ImageArea", "props": { "alt": "initial", "src": "http://blog-src.b0.upaiyun.com/taohe/dev/editPage/administrator/1/layout/tmp/2f7ddd3dc6d11f62c204a2fd2adf0f47"}}'
        );
      case "纵向布局":
        return JSON.parse(
  ...
  render() {
    return (
      <Dropdown overlay={menu(this.add)} trigger={["click"]}>
        <div
          style={{ cursor: "pointer", minHeight: 23 }}
          onMouseOut={this.hiddenSelf}
          onMouseOver={this.showSelf}
        >
          <div
            hidden={!this.props.permanent && this.state.hidden}
            style={{ textAlign: "center", width: "100%", position: "relative" }}
          >
            <IconButton
              aria-label="Add an alarm"
              style={{ blackground: "black", height: 22 }}
            >
              <AddIcon />
            </IconButton>
            <div
